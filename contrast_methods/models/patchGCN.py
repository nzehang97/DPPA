...
class PatchGCN(torch.nn.Module):
    def __init__(self, input_dim=2227, num_layers=4, edge_agg='spatial', multires=False, resample=0,
        fusion=None, num_features=768, hidden_dim=128, linear_dim=64, use_edges=False, pool=False, dropout=0.25, n_classes=4):
        ...
        if self.resample > 0:
            self.fc = nn.Sequential(*[nn.Dropout(self.resample), nn.Linear(768, 256), nn.ReLU(), nn.Dropout(0.25)])
        else:
            self.fc = nn.Sequential(*[nn.Linear(768, 128), nn.ReLU(), nn.Dropout(0.25)])
    ...
    def forward(self,  graph_data, device):
        ...
        edge_index = data['edge_index'].squeeze().to(device)

        edge_attr = None

        x = self.fc(data['x'].squeeze().to(device))
        x_ = x 
