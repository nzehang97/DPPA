...
class LoadData_CLAM1(Dataset):
    def __init__(self, txt_path, train=False):
        ...
        for slide in slides:
            s = slide.split('.')[0]
            data = torch.load('./0_graph_data/' + s + '.pt')
            self.graph[slide] = {'x': data.x, 'edge_index': data.edge_index}

    def __getitem__(self, index):
        slide, label = self.data[index]
        label = torch.tensor([int(i) for i in label])
        graph_data = self.graph[slide]
...
def predict(args, ii, model, device, weight='early_stop'):
    model.to(device)
    ...
    weight_path = os.path.join(args.weight_path, args.task, f'PatchGCN/weight_fold_{ii}.pth')
...
if __name__ == '__main__':
    ...
    for ii in range(args.k):
        ...
        model_dict = {'num_layers': 4, 'edge_agg': 'spatial', 'resample': 0.00, 'n_classes': 2}
        model = PatchGCN(**model_dict).to(device)
