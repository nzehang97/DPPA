...
def predict(args, ii, model, device):
    ...
    weight_path = os.path.join(args.weight_path, args.task, f'TransMIL/weight_fold_{ii}.pth')
    ...
    with torch.no_grad():
        for step, data in enumerate(test_loader):
            ...
            out, _ = model(feature_ct.to(device))
            out = softmax(out)
            label = label.flatten()
...
if __name__ == '__main__':
    ...
    for ii in range(args.k):
        ...
        model = TransMIL(n_classes=2, device=device, input_ch=768)
