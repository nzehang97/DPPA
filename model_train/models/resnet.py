...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        if dropblock_prob > 0.0:
            self.dropblock1 = nn.Dropout(dropblock_prob)
            if radix == 1:
                self.dropblock2 = nn.Dropout(dropblock_prob)
            self.dropblock3 = nn.Dropout(dropblock_prob)
...
def resnet18(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(
            resnest_model_urls['resnet18'], progress=True, check_hash=True))
    return model
