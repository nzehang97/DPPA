...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
                   # top_left=(25000, 100000), bot_right=(60000, 130000),
        ...
        Args:
            ...
            blank_canvas (bool): Whether to use a blank canvas to draw the clust_map (vs. using the original slide)
            canvas_color (tuple of uint8): Canvas color
            alpha (float [0, 1]): blending coefficient for overlaying clust_map onto original slide
            ...
            custom_downsample (int): additionally downscale the clust_map by specified factor
        ...
        ##### calculate size of clust_map and filter coordinates/scores outside specified bbox region #####
        if top_left is not None and bot_right is not None:
            scores = np.load(f'{name}.npy')
        ...
        ######## calculate the clust_map of raw attention scores (before colormap)
        # by accumulating scores over overlapped regions ######
        
        # clust_map overlay: tracks attention score over each pixel of clust_map
        # overlay counter: tracks how many times attention score is accumulated over each pixel of clust_map
        overlay = np.full(np.flip(region_size), 0).astype(np.float32)
        counter = np.full(np.flip(region_size), 0).astype(np.uint8)
        count = 0
